...
representation_model = "d2v representation name"
arg_tsv_file = "dataset/dataset.tsv"
...
#weight_relations = {"news_term": 1, "news_news": 1, "news_features":}
	...
	for ngr in arg_ngram_range:
		for mdf in arg_min_df:
			for nrm in arg_norm:
				...
				params_pulp.write('id\tdataset\trepresetation\tfold\n')

				params_prop = open(dir_params_prop.format(op, exp_id), 'w')
				params_prop.write('id\tpulp_id\tfold\tmi\tmax_iter\tlimiar_conv\tweight_relations\tnetwork_relation\tk\n')
				...
				for p in range(3):
					...
					for i in range(10):
						...
						pulp_id = md5('{} {} {}'.format(arg_tsv_file, representation_model, fold).encode()).hexdigest()
						params_pulp.write('{}\t{}\t{}\t{}\n'.format(pulp_id, arg_tsv_file, representation_model, fold))
						...
						name_dir_adj_matrix = dir_adj_matrix.format(op, exp_id)
						dir_params_pulp_format = dir_params_pulp.format(op, exp_id)
						f = open(file_name, 'a')
						f.write("python3 PU-LP.py {} {} {} > logs/{}_{}-pulp.log\n". format(pulp_id, dir_params_pulp_format, pu_lp_output, exp_id, pulp_id))
						...
						for k in arg_k:
							...
							for net in range(len(net_metadata)):
								...
								for mi in arg_mi:
									...
									prop_id = md5('{} {} {} {} {} {} {} {}'.format(pulp_id, fold, mi, arg_max_iter, agr_limiar_conv, net_metadata.weight_relations[net], network_relation, k).encode()).hexdigest()
		
									params_prop.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(prop_id, pulp_id, fold, mi, arg_max_iter, agr_limiar_conv, net_metadata.weight_relations[net], network_relation, k))													

									f.write("java -Xmx5G -cp ClassificadorTrasdutivo.jar algoritmos.RegularizarFakeNews {} {} {} {} {} {} {} > logs/{}_{}-gnm.log\n".format('gn', prop_id, pulp_id, relations.format(op, exp_id, k, network_relation), 
										dir_params_prop.format(op, exp_id), output_heterg_gnm.format(op, exp_id, mi, prop_id), dir_pulp_output.format(op, exp_id, p+1), prop_id, pulp_id)) 
									
									
									f.write("python3 results_GNM.py {} {} {} {} {} > logs/{}_{}-resuts-gnm.log\n".format(exp_id, pulp_id, prop_id, output_heterg_gnm.format(op, exp_id, mi, prop_id),  dir_bow_op.format(op, exp_id), prop_id, pulp_id))

								
								
								f.write("java -Xmx5G -cp ClassificadorTrasdutivo.jar algoritmos.RegularizarFakeNews {} {} {} {} {} {} {} > logs/{}_{}-gnm.log\n".format('lp', prop_id, pulp_id, relations.format(op, exp_id, k, network_relation), 
									dir_params_prop.format(op, exp_id), output_heterg_lphn.format(op, exp_id, prop_id), dir_pulp_output.format(op, exp_id, p+1), prop_id, pulp_id)) 

								
								f.write("python3 results_LPHN.py {} {} {} {} {} > logs/{}_{}-results-lp.log\n".format(exp_id, pulp_id, prop_id, output_heterg_lphn.format(op, exp_id, prop_id), dir_bow_op.format(op, exp_id), prop_id, pulp_id))
